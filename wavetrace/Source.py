"""CSI sources: every transport that can hand the pipeline a stream of CsiFrames.

The rest of the pipeline is source-agnostic, so a source only has to yield CsiFrames:
  * SyntheticSource - an in-memory frame list, generated by the fixtures for dev and CI.
  * RecordingSource - replays a directory written by `saveRecording`.
  * UdpSource - the batched-UDP backhaul the ESP32 mesh sends on.
  * SerialReader - esp-csi over USB serial, one ESP per node; the bring-up path for one or two
    nodes before the UDP mesh exists. Needs `pyserial`.
  * NexmonSource - 5 GHz CSI from a Raspberry Pi running nexmon_csi. Needs `csiread`.

A recording is grid.npy (F,A,S) complex64 + t.npy (F,) + node_id.npy (F,) + meta.json, and costs
O(F·A·S) to write or read.
"""
...
# ver 3 = int16 I/Q (Pi 5 GHz node: keeps the full amplitude range the weapon σ² feature needs).
...
class UdpSource(CsiSource):
    """Receive batched-UDP CSI. frames() binds 0.0.0.0:port and yields until
...
class NexmonSource(CsiSource):
    ...
    Reads nexmon-encapsulated CSI from a pcap file (`pcap_path`, offline) or from a live tcpdump
    on `iface`. `csiread.Nexmon` does the decoding, so only `_decodeFile` changes if your csiread
    version or chip differs.

    node_id defaults to 100: ESP32 nodes are 1..6 and everything downstream reads node_id >= 100
    as the 5 GHz band."""
...
def buildCsiSource(options) -> CsiSource:
    ...
    if options.synthetic:
        ...
        if parseTimeSpans(options.weapon) and options.weapon_depth <= 0.0:
            # depth 0 injects no signal, so the weapon windows would all be one class
