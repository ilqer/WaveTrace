...
def test_carry_groups_parses_weapon_session_ids():
    ...
    np.testing.assert_array_equal(_carryGroups(sess), ["chest", "chest", "waist"])


def test_carry_groups_none_for_non_weapon_ids():
    # presence-style ids without the <subject>_<carry>_s<n> shape get no carry axis
    assert _carryGroups(np.array(["sessionA", "sessionB"])) is None
    assert _carryGroups(np.array(["p0_chest_x0", "p0_chest_x1"])) is None  # not s<digit>
